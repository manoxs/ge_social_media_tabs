...
class dc_jqsocialmediatabs_widget extends WP_Widget {
    ...
    function dc_jqsocialmediatabs_widget() {
		...
		$def_tabs = 'facebook,twitter,instagram,rss,youtube,flickr,pinterest';
	...
	function widget($args, $instance){
		...
		$dcsmt_tabs = '';
		$i = 0;
		$deftabs = explode(',','facebook,twitter,instagram,rss,youtube,flickr,pinterest,extra,custom');
    ...
    function update( $new_instance, $old_instance ) {
		...
		$instance['instagramId'] = strip_tags(stripslashes($new_instance['instagramId']) );
		$instance['instagramTitle'] = strip_tags( stripslashes($new_instance['instagramTitle']) );
		$instance['nInstagram'] = strip_tags( stripslashes($new_instance['nInstagram']) );
		...
		$deftabs = explode(',','facebook,twitter,instagram,rss,youtube,flickr,pinterest,custom');
		foreach($deftabs as $k=>$tabv){
			if($tabv != ''){
				$k = $k+1;
    ...
    function form($instance) {
		...
		$nInstagram = isset( $instance['nInstagram']) ? $instance['nInstagram'] : '12';
	...
	<?php
		$deftabs = explode(',','facebook,twitter,instagram,rss,youtube,flickr,pinterest');
		foreach($deftabs as $k=>$tabv){
			if($tabv != ''){
				$x = $k;
				$k = $k+1;
	...
	<p class="dcwp-row">
		<label for="<?php echo $this->get_field_id('instagramId'); ?>"><?php _e('ID:') ?></label>
		<input type="text" id="<?php echo $this->get_field_id('instagramId'); ?>" name="<?php echo $this->get_field_name('instagramId'); ?>" value="<?php echo $instagramId; ?>" />
	</p>
	<p class="dcwp-row">
		<label for="<?php echo $this->get_field_id('instagramTitle'); ?>"><?php _e('Title:') ?></label>
		<input type="text" id="<?php echo $this->get_field_id('instagramTitle'); ?>" name="<?php echo $this->get_field_name('instagramTitle'); ?>" value="<?php echo $instagramTitle; ?>" />
	</p>
	<p class="dcwp-row">
	  <label for="<?php echo $this->get_field_id('nInstagram'); ?>"><?php _e( 'Results:' , 'dcjq-social-tabs' ); ?></label>
		<input type="text" id="<?php echo $this->get_field_id('nInstagram'); ?>" name="<?php echo $this->get_field_name('nInstagram'); ?>" value="<?php echo $nInstagram; ?>" size="4" />
	</p>
	<p class="dcwp-row">
	...
	function dcsmt_tab_options($id, $label, $value){
		...
		$instagram = $value == 'instagram' ? '<option value="instagram" selected="selected">':'<option value="instagram">';
		...
		$select .= $instagram.'Instagram </option>';
...
<script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src="//platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document,"script","twitter-wjs");</script>
	...
	/*Instagram*/
	function dcsmt_inc_instagram($args, $instance){

		extract( $args );
		$widget_options = wp_parse_args( $instance, $this->defaults );
		extract( $widget_options, EXTR_SKIP );
		$id = $widget_options['instagramId'];
		$rssId = 'http://instagram.com/'.$id;
		$icon_close = '<a href="#" class="dcsmt-close dcsmt-close-tab"></a>';
		$title = $widget_options['instagramTitle'] == '' ? '' : '<h3><a href="http://instagram.com/'.$id.'/">'.$widget_options['instagramTitle'].'</a>'.$icon_close.'</h3>';
		$nInstagram = $widget_options['nInstagram'];
		$tab = '';
		if($id != ''){
			...
			$tab = '<script type="text/javascript">
			jQuery(document).ready(function($) {
			$(".tab-instagram").jCarouselLitedcsmt({
			...
			$(".dcsmt-instagram-item a").each(function(){
				if(!$(this).hasClass("hrefconvert")){
				var href = $(this).attr("href");
				href = href.replace("/pin/","http://instagram.com/pin/");
			...
			$tab .= '<div class="tab-instagram tab-inner" style="width: '.$widget_options['width'].';">'.$title;
			$rss = fetch_feed($rssId);
			print_r($rss);
			$maxitems = $rss->get_item_quantity((int)$nInstagram);
			$rss_items = $rss->get_items(0,$maxitems);
			
			
			$tab .= '<div class="carousel" style="width: '.$widget_options['width'].';">'."\n";
			$tab .= '<ul class="list-dcsmt-instagram">';
			$count = 1;
				foreach ( $rss_items as $item ) :
					
					$time = wpcom_time_since(strtotime($item->get_date()));
					$tab .= '<li class="dcsmt-instagram-item carousel-slide" style="width: '.$widget_options['width'].';"><div class="pin">';
